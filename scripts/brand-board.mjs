...
// masthead
parts.push(`<text x="${M}" y="96" font-family="Inter" font-size="40" font-weight="700" fill="${TITLE}">${esc("FEL DRONE — identity system v5")}</text>`);
parts.push(
...
parts.push(
  `<text x="${M}" y="160" font-family="Inter" font-size="14" fill="${SUB}">${esc("v5 refinement \u2014 3\u00b0 forward lean + corrected mark\u2192word optical gap \u00b7 one geometry source: scripts/brand-gen.mjs (npm run brand:check keeps files in sync)")}</text>`,
);
...
parts.push(brandImg("public/brand/fel-drone-horizontal.svg", M + (PW - 1240) / 2, p1y + (p1h - 251.1) / 2, 251.1, 1240));
...
// footer
parts.push(
  `<text x="${M}" y="${H - 44}" font-family="Inter" font-size="13" fill="${FOOT}">${esc("regenerated by scripts/brand-board.mjs from scripts/brand-gen.mjs output \u00b7 identity v5.2 \u00b7 2026-09 \u00b7 geometry: v3 skeleton + v4 lab + v5 motion + v5.2 one-word lockup")}</text>`,
);
parts.push(
