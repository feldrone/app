...
// masthead
parts.push(`<text x="${M}" y="96" font-family="Inter" font-size="40" font-weight="700" fill="${TITLE}">${esc("FELDRONE — identity system v6")}</text>`);
parts.push(
  `<text x="${M}" y="134" font-family="Inter" font-size="17" fill="${SUB}">${esc("the FD counter monogram: a flat-shouldered D ring whose shared stem, top edge and floating mid arm are simultaneously the F \u00b7 T = 24 one weight \u00b7 monochrome first")}</text>`,
);
parts.push(
  `<text x="${M}" y="160" font-family="Inter" font-size="14" fill="${SUB}">${esc("v6.0 redesign per client spec \u2014 strict 136\u00d7160 grid, module 4u, zero dots, one-word FELDRONE at 10u \u00b7 one geometry source: scripts/brand-gen.mjs")}</text>`,
);
...
// footer
parts.push(
  `<text x="${M}" y="${H - 44}" font-family="Inter" font-size="13" fill="${FOOT}">${esc("regenerated by scripts/brand-board.mjs from scripts/brand-gen.mjs output \u00b7 identity v6.0 \u00b7 2026-09 \u00b7 FD counter monogram per client brand spec of 2026-09-13 \u00b7 grid 136\u00d7160 module 4u")}</text>`,
);
parts.push(
