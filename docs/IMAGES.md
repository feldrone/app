...
All photography used on the FEL DRONE website is licensed stock imagery from
[Pexels](https://www.pexels.com) under the **[Pexels License](https://www.pexels.com/license/)**:
free for commercial use, no attribution required, modification (cropping /
color treatment) permitted. Images are served from the Pexels CDN with
server-side cropping and compression (see `src/lib/images.ts`).

If the company later prefers fully self-hosted assets, download the originals
from the page URLs below (or the matching official press material), optimize
to WebP/AVIF and swap the URL builders in `src/lib/images.ts` for local paths.
