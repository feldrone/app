/**
 * FEL DRONE — quote-request core (framework-free, dependency-free).
 *
...
function fileStore(file) {
  // Dev-only JSON-lines store so local testing survives restarts.
  const fs = () => fsNode;
  return {
    kind: "file",
    async save(record) {
    ...
    async setStatus(id, status) {
      ...
      // rewrite everything (dev store, tiny)
    ...
    return {
      async check(ip) {
        ...
        try {
          ...
          return true; // limiter down → do not block real customers; log server-side
        }
      },
    };
  }

  const hits = new Map(); // ip -> {minute:[], day:[]}
  return {
    async check(ip) {
      const now = Date.now();
...
export async function notifyCompany(record, env = process.env) {
  ...
  const text = [
    "Nouvelle demande reçue via feldrone.dz",
    "",
    `Référence : ${record.id}`,
    `Nom      : ${record.name}`,
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from, to: to.split(","), subject: `Demande ${record.service} — ${record.name}`, text }),
...
export function allowedOrigin(env, requestOrigin) {
  ...
  // Same-origin (no header sent by fetch when same-site) is handled by absence.
...
export function readJsonBody(req, maxBytes = 32 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    const chunks = [];
    req.on("data", (c) => {
      if (tooBig) return; // drain the rest, respond once
      size += c.length;
      if (size > maxBytes) {
        tooBig = true;
...
/** Warm-instance singletons: in-memory fallbacks persist across requests. */
...
/**
 * POST /api/quote — the public intake endpoint.
 * Returns {status, body} so any runtime (Vercel res, node res) can shape it.
 */
export async function intakeQuote(req, env = process.env, services = { notify: notifyCompany }) {
  ...
  // Notification never blocks or fails the intake.
...
/** GET/PATCH /api/quote-requests — private, Bearer ADMIN_TOKEN. */
